...
(*petits rajouts du plaisir pour écrire gaiement les floats*)

(* pour appeler un float qui sera stoké en variable à la fin *)
let immf x stack =
  if not (List.mem x !stack) then stack := x::!stack;
  !%("f" ^ (string_of_float x))




let rereg r = fun fmt () -> fprintf fmt "%s" r
...
let pushsd a = inline "\tsubq $8, %rsp\n" ++ movsd a "(%rsp)"
let popsd a = movsd (rereg "(%rsp)") a ++ inline "\taddq $8, %rsp\n"

let cvtsi2sdq s d = ins "cvtsi2sdq %a, %s" s () d
let cvttsd2siq s d = ins "cvttsd2siq %a, %s" s () d

let addsd s d = ins "addsd %a, %s" s () d
let subsd s d = ins "subsd %a, %s" s () d
let mulsd s d = ins "mulsd %a, %s" s () d
let divsd d s = ins "divsd %a, %s" s () d